...
function openTreeItem(offset, provider) {
    ...
    vscode_1.env.openExternal(treeItem.label ? treeItem.label : '');
}
...
function writeDbFile(savePath, host) {
    vscode_1.window.showInformationMessage('Exporting database from ' + host);
    lando_1.dbExportCustom(extension_1.getWorkspaceFolderPath(), host, 'tmp_export.sql');
    fs_1.rename(extension_1.getWorkspaceFolderPath() + '/' + 'tmp_export.sql.gz', savePath + '.gz', (err) => {
        if (err) {
            extension_1.outputChannel.append('Error: Moving export file to save location: ' + err);
            vscode_1.window.showInformationMessage('Failed to move export to save location');
        }
...
function pickWorkspaceFolder() {
    vscode_1.window.showWorkspaceFolderPick().then((workspaceFolder) => {
        if (workspaceFolder) {
            vscode_1.workspace.getConfiguration('lando-ui.workspaceFolder').update('default', workspaceFolder.uri.fsPath, vscode_1.ConfigurationTarget.Workspace);
        }
    });
}
...
function determineWorkspaceFolder() {
    var defaultWorkspaceFolderPath = vscode_1.workspace.getConfiguration('lando-ui.workspaceFolder').get('default');
    if (defaultWorkspaceFolderPath != undefined && defaultWorkspaceFolderPath != '') {
        if (isWorkspaceFolder(defaultWorkspaceFolderPath)) {
            ...
            setButtonTo('pick');
            vscode_1.window.showWarningMessage('Your current default Workspace folder does not exists in this Workspace. Please select one to be the default.', ...['Select Default Folder']).then((selection) => {
...
function setButtonTo(mode) {
    switch (mode) {
        case 'start':
            exports.toggleButton.text = 'Lando Start';
            exports.toggleButton.command = 'lando-ui.start';
            break;
        case 'starting':
            exports.toggleButton.text = 'Lando Starting...';
            exports.toggleButton.command = '';
            break;
        case 'stop':
            exports.toggleButton.text = 'Lando Stop';
            exports.toggleButton.command = 'lando-ui.stop';
            break;
        case 'stopping':
            exports.toggleButton.text = 'Lando Stopping...';
            exports.toggleButton.command = '';
            break;
        case 'restarting':
            exports.toggleButton.text = 'Lando Restarting...';
            exports.toggleButton.command = '';
            break;
        case 'rebuilding':
            exports.toggleButton.text = 'Lando Rebuilding...';
            exports.toggleButton.command = '';
            break;
        case 'destroying':
            exports.toggleButton.text = 'Lando Destroying...';
            exports.toggleButton.command = '';
            break;
        case 'init':
            exports.toggleButton.text = 'Lando Init';
            exports.toggleButton.command = 'lando-ui.init';
            break;
        case 'pick':
            exports.toggleButton.text = 'Lando Pick Folder';
            exports.toggleButton.command = 'lando-ui.pickWorkspaceFolder';
            break;
        default:
            exports.toggleButton.text = 'Loading...';
            exports.toggleButton.command = '';
            break;
    }
}
...
const fs_1 = __webpack_require__(/*! fs */ "fs");
function init() {
    var terminal = vscode_1.window.createTerminal('Lando UI');
    terminal.show();
    terminal.sendText('lando init');
}
exports.init = init;
function start(dir) {
    extension_1.showOutput();
    ...
    child.stdout.on('data', (data) => {
        if (data.includes('Could not find app in this dir or a reasonable amount of directories above it!')) {
            ...
            extension_1.setButtonTo('init');
        }
        if (data.includes('Starting app')) {
            vscode_1.window.showInformationMessage('Starting the Lando app ' + extension_1.getCurrentAppName());
            extension_1.setButtonTo('starting');
        }
        if (data.includes('Your app has started up correctly')) {
            vscode_1.window.showInformationMessage('The Lando app ' + extension_1.getCurrentAppName() + ' started successfully');
            extension_1.setButtonTo('stop');
            vscode_1.commands.executeCommand('lando-ui.info-refresh');
...
exports.start = start;
function stop(dir, appName) {
    extension_1.showOutput();
    var isCurrentApp = appName == extension_1.getCurrentAppName();
    ...
    child.stdout.on('data', (data) => {
        ...
        if (data.includes('Stopping app')) {
            vscode_1.window.showInformationMessage('Stopping the Lando app ' + (appName || extension_1.getCurrentAppName()));
            if (isCurrentApp)
                extension_1.setButtonTo('stopping');
        }
        if (data.includes('stopped')) {
            vscode_1.window.showInformationMessage('The Lando app ' + (appName || extension_1.getCurrentAppName()) + ' stopped successfully');
            if (isCurrentApp)
                extension_1.setButtonTo('start');
            vscode_1.commands.executeCommand('lando-ui.info-refresh');
            vscode_1.commands.executeCommand('lando-ui.list-refresh');
        }
        extension_1.outputChannel.append(`${stripAnsi.default(data)}`);
...
function restart(dir) {
    ...
    child.stdout.on('data', (data) => {
        ...
        if (data.includes('just so we can start it up again') || data.includes('Stopping and restarting your app')) {
            vscode_1.window.showInformationMessage('Restarting the Lando app ' + extension_1.getCurrentAppName());
            extension_1.setButtonTo('restarting');
        }
        if (data.includes('Your app has started up correctly')) {
            vscode_1.window.showInformationMessage('The Lando app ' + extension_1.getCurrentAppName() + ' has restarted successfully');
            extension_1.setButtonTo('stop');
            vscode_1.commands.executeCommand('lando-ui.info-refresh');
            vscode_1.commands.executeCommand('lando-ui.list-refresh');
        }
...
function rebuild(dir) {
    vscode_1.window.showQuickPick(['Cancel', 'Confirm'], { placeHolder: 'Are you sure you want to rebuild your app?' }).then((selected) => {
        if (selected == 'Confirm') {
            extension_1.showOutput();
            const child = child_process_1.exec('lando rebuild -y', { cwd: dir });
            child.stdout.on('data', (data) => {
                if (data.includes('Could not find app in this dir or a reasonable amount of directories above it!')) {
                    vscode_1.window.showWarningMessage('Please initiate a lando project: ' + data);
                    extension_1.setButtonTo('init');
                ...
                if (data.includes('Rebuilding app')) {
                    vscode_1.window.showInformationMessage('Rebuilding the Lando app ' + extension_1.getCurrentAppName());
                    extension_1.setButtonTo('rebuilding');
                }
                if (data.includes('Your app has started up correctly')) {
                    vscode_1.window.showInformationMessage('The Lando app ' + extension_1.getCurrentAppName() + ' has rebuilt successfully');
                    extension_1.setButtonTo('stop');
                    vscode_1.commands.executeCommand('lando-ui.info-refresh');
                    vscode_1.commands.executeCommand('lando-ui.list-refresh');
                }
...
function destroy(dir) {
    vscode_1.window.showQuickPick(['Cancel', 'Confirm'], { placeHolder: 'Are you sure you want to destroy your app?' }).then((selected) => {
        if (selected == 'Confirm') {
            extension_1.showOutput();
            const child = child_process_1.exec('lando destroy -y', { cwd: dir });
            child.stdout.on('data', (data) => {
                if (data.includes('Could not find app in this dir or a reasonable amount of directories above it!')) {
                    vscode_1.window.showWarningMessage('Please initiate a lando project: ' + data);
                    extension_1.setButtonTo('init');
                ...
                if (data.includes('dustbin of history')) {
                    vscode_1.window.showInformationMessage('Destroying the Lando app ' + extension_1.getCurrentAppName());
                    extension_1.setButtonTo('destroying');
                }
                if (data.includes('App destroyed')) {
                    vscode_1.window.showInformationMessage('The Lando app ' + extension_1.getCurrentAppName() + ' has been destroyed');
                    extension_1.setButtonTo('start');
                    vscode_1.commands.executeCommand('lando-ui.info-refresh');
                    vscode_1.commands.executeCommand('lando-ui.list-refresh');
                }
...
exports.info = info;
function list(appName) {
    var command = 'lando list --format json';
    if (appName != undefined)
        command = command + ' --filter "app=' + appName + '"';
...
function dbExportCustom(dir, host, filePath) {
    let command = 'lando db-export';
    if (host)
        command += ' -h ' + host;
    if (filePath)
        command += ' "' + filePath + '"';
    var stdout = child_process_1.execSync(command, { cwd: dir });
    if (stdout.includes('Could not find app in this dir or a reasonable amount of directories above it!')) {
        vscode_1.window.showWarningMessage('Please initiate a lando project');
        extension_1.setButtonTo('init');
    }
...
exports.dbExportCustom = dbExportCustom;
...
class LandoInfoProvider {
    ...
    parseTree() {
        ...
        if (Object.keys(json.parse(this.text)).length <= 0) {
            ...
            this.text = lando_1.addWorkspaceFolderName(this.text);
        }
    ...
    getTreeItem(offset) {
        ...
        if (valueNode) {
            ...
            let treeItem = new vscode_1.TreeItem(label, hasChildren ? (valueNode.type === 'object' || valueNode.type === 'array' ? vscode_1.TreeItemCollapsibleState.Expanded : vscode_1.TreeItemCollapsibleState.Collapsed) : vscode_1.TreeItemCollapsibleState.None);
...
class LandoListProvider {
    ...
    getTreeItem(offset) {
        ...
        if (valueNode) {
            let hasChildren = valueNode.type === 'object' || valueNode.type === 'array';
            let label = this.getLabel(valueNode);
            let treeItem = new vscode_1.TreeItem(label, hasChildren ? (valueNode.type === 'object' ? vscode_1.TreeItemCollapsibleState.Expanded : vscode_1.TreeItemCollapsibleState.Collapsed) : vscode_1.TreeItemCollapsibleState.None);
            if (label.match(/^service_/)) {
